...
def calculate_world_height(x: float, y: float) -> float:
    ...
    # Grotto Alcove: smoothly carve into the cliff face at (-10.0, -16.0)
    d_grotto = math.hypot((x - (-10.0)) * 1.0, (y - (-16.0)) * 1.2)
    grotto_carve = 0.0
    if d_grotto < 16.0:
        grotto_carve = -math.cos(d_grotto / 16.0 * math.pi * 0.5) * 7.5
    ...
    final_h = (h_base + h_peak1 + h_peak2 + h_ridge + tarn_bowl +
               ...
               river_trough + grotto_carve + n1 + n2 + n3)
    return max(-8.0, final_h)

for y in range(-30, -5, 3):
    print(f"y={y:3d}: h(-10, {y}) = {calculate_world_height(-10.0, y):.2f}m")
