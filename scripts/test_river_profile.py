...
def calculate_world_height(x: float, y: float) -> float:
    ...
    river_trough = 0.0
    if -68.0 < y < -12.0:
        t = (y - (-16.0)) / (-68.0 - (-16.0)) # 0 at cave to 1 at lake
        t = max(0.0, min(1.0, t))
...
print("--- River Bed Profile from Cave to Lake ---")
for y in range(-15, -69, -5):
    t = (y - (-16.0)) / (-68.0 - (-16.0))
    t = max(0.0, min(1.0, t))
    rx = (1.0 - t) * (-10.0) + t * (18.0 * math.sin((y + 15.0) * 0.12))
    h = calculate_world_height(rx, y)
    print(f"y={y:3d} (x={rx:5.1f}): Bed Height = {h:5.2f}m")
