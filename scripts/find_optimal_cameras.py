...
def check_ray(cx, cy, cz, tx, ty, tz, steps=60):
    for i in range(1, steps):
        ...
        if rz <= gh:
            return False, t, gh, rz
    return True, 1.0, 0, 0

print("--- Testing Candidates for Tarn (Target: -70.0, 75.0, 40.0) ---")
# Alpine Tarn water is at z = 39.5m, center is (-70.0, 75.0)
for y_c in [52.0, 56.0, 60.0]:
    for x_c in [-70.0, -85.0, -55.0]:
        gh = calculate_world_height(x_c, y_c)
        cz = gh + 4.5
        ok, frac, hit_gh, hit_rz = check_ray(x_c, y_c, cz, -70.0, 75.0, 40.0)
        if ok:
            print(f"  ✓ Tarn Cam at ({x_c}, {y_c}, {cz:.1f}) -> Ground={gh:.1f}, Clear to Tarn!")

print("\n--- Testing Candidates for Waterfall Canyon (Target: -25.0, 45.0, 42.0) ---")
# Looking up canyon at cascades
for y_c in [12.0, 18.0, 24.0]:
    for x_c in [-5.0, 0.0, 5.0]:
        gh = calculate_world_height(x_c, y_c)
        cz = gh + 4.0
        ok, frac, hit_gh, hit_rz = check_ray(x_c, y_c, cz, -25.0, 45.0, 42.0)
        if ok:
            print(f"  ✓ Waterfall Cam at ({x_c}, {y_c}, {cz:.1f}) -> Ground={gh:.1f}, Clear to Cascade!")

print("\n--- Testing Candidates for Lake Solitude (Target: 12.0, -105.0, 13.0) ---")
# Solitary Islet is at (12.0, -105.0) in the lake
for y_c in [-75.0, -78.0, -82.0]:
    for x_c in [-25.0, -15.0, 0.0]:
        gh = calculate_world_height(x_c, y_c)
        cz = max(13.5, gh + 3.0)
        ok, frac, hit_gh, hit_rz = check_ray(x_c, y_c, cz, 12.0, -105.0, 13.0)
        if ok:
            print(f"  ✓ Lake Cam at ({x_c}, {y_c}, {cz:.1f}) -> Ground={gh:.1f}, Clear to Lake Islet!")

print("\n--- Testing Candidates for Valley River (Target: 0.0, -45.0, 12.0) ---")
for y_c in [-28.0, -32.0, -36.0]:
    for x_c in [15.0, 22.0, 28.0]:
        gh = calculate_world_height(x_c, y_c)
        cz = gh + 4.0
        ok, frac, hit_gh, hit_rz = check_ray(x_c, y_c, cz, 0.0, -45.0, 12.0)
        if ok:
            print(f"  ✓ River Cam at ({x_c}, {y_c}, {cz:.1f}) -> Ground={gh:.1f}, Clear to River!")
